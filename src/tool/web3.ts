...
export const web3Tools = {
  readChain: tool({
    ...
    inputSchema: z.object({
      chainId: z.number().describe("Chain ID (1=ETH, 42161=ARB, 137=MATIC, 8453=BASE, 10=OP, 56=BSC)"),
      address: z.string().describe("Address to query"),
      method: z.string().optional().describe("Contract method name to call. If omitted, returns ETH balance."),
  ...
  sendTransaction: tool({
    description: "Prepare a blockchain transaction for review. Actual sending requires explicit approval.",
    inputSchema: z.object({
      chainId: z.number(),
