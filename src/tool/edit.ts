...
export const editTools = {
  replaceFileContent: tool({
    description:
      "Replace a specific range of lines in a file with new content. Use this instead of full file rewrites for surgical edits.",
  ...
  multiReplaceFileContent: tool({
    ...
    execute: async ({
      ...
      path: string;
      replacements: Array<{ startLine: number; endLine: number; content: string }>;
    }) => {
      const p = safeResolve(path, ws());
      const fileContent = await fs.readFile(p, "utf-8");
  ...
  createDiff: tool({
    description:
      "Generate a unified diff patch for proposed file changes. Use this to preview what would change, then applyDiff to commit.",
    inputSchema: z.object({
      path: z.string().describe("File path relative to project root"),
