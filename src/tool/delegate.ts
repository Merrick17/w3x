...
export const delegateTool = {
  delegateTask: tool({
    description: "Spawn a read-only sub-agent to handle complex, multi-step research or exploration tasks independently. Sub-agents can read files, search code, and explore the codebase but cannot modify anything.",
    inputSchema: z.object({
      task: z.string().describe("Detailed description of what the sub-agent should investigate"),
      agentType: z.enum(["general", "explorer", "reviewer"]).optional().describe("Type of sub-agent to spawn (default: general)"),
    }),
    execute: async ({ task, agentType }: { task: string; agentType?: "general" | "explorer" | "reviewer" }) => {
      if (!_currentModel) return { success: false, error: "No model available for sub-agent delegation" };
