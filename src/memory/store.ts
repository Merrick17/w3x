...
export async function saveFact(key: string, value: string): Promise<void> {
  ...
  await db.run(
    "INSERT OR REPLACE INTO facts (key, value, saved_at) VALUES (?, ?, ?)",
    key,
    value,
...
export async function loadRecentSessions(
  limit = 3,
): Promise<Array<{ summary: string; savedAt: string }>> {
  const db = await getDb();
  const rows = await db.all(
    "SELECT summary, saved_at FROM sessions ORDER BY saved_at DESC LIMIT ?",
    limit,
  );
  return rows.map((row) => ({
    summary: row.summary,
