...
export const solanaTools = {
  getSolanaBalance: tool({
    ...
    execute: async ({ addr, rpcUrl }) => {
      ...
      try {
        ...
        return { error: `Failed to fetch balance: ${err instanceof Error ? err.message : String(err)}` };
      }
    }
  }),
  ...
  resolveSNS: tool({
    ...
    execute: async ({ domain }) => {
      ...
      return {
        ...
        message: 'SNS resolution requires additional specialized libraries. This is a placeholder for the agent to implement further.'
      };
    }
  }),
  ...
  analyzeSolanaTransaction: tool({
    ...
    execute: async ({ sig, rpcUrl }) => {
      const rpc = createSolanaRpc(rpcUrl || DEFAULT_RPC);
      try {
        const tx = await rpc.getTransaction(signature(sig), { maxSupportedTransactionVersion: 0, encoding: 'json' }).send();
        ...
        return { error: `Failed to fetch transaction: ${err instanceof Error ? err.message : String(err)}` };
      }
    }
  }),
